...
if __name__ == "__main__":
    ...
    # model.train(N_Iter = 1*10**2, learning_rate=1e-3)
    ...
    # 使用 tf.transpose() 转置张量
    plt.plot(tf.transpose(t_test[0:1, :, 0]), tf.transpose(Y_pred[0:1, :, 0]), 'b', label='Learned $u(t,X_t)$')
    plt.plot(tf.transpose(t_test[0:1, :, 0]), tf.transpose(Y_test[0:1, :, 0]), 'r--', label='Exact $u(t,X_t)$')
